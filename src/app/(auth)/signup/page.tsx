...
export default function SignupPage() {
  const [selectedRole, setSelectedRole] = useState<"recruiter" | "employee">(
    "employee"
  )

  const setSelectedRoleHandler = (role: "recruiter" | "employee") => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-start gap-8">
      ...
      <div className="flex flex-row gap-4 items-center justify-center w-full">
        <RoleSelector
          onSelect={() => setSelectedRoleHandler("employee")}
      ...
      {selectedRole === "employee" ? <SignUpFormEmployee /> : <SignUpFormEmployer />}
    </div>
  )
}
